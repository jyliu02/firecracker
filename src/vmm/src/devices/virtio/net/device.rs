...
use crate::devices::virtio::gen::virtio_net::{
    virtio_net_hdr_v1, VIRTIO_NET_F_CSUM, VIRTIO_NET_F_CTRL_VQ, VIRTIO_NET_F_GUEST_CSUM,
...
#[derive(Debug)]
pub struct CtrlCommand<'a> {
    pub hdr: CtrlHdr,
    pub data: Option<Vec<u8>>,
    pub ack: GuestAddress,
    pub mem: &'a GuestMemoryMmap,
}

impl<'a> CtrlCommand<'a> {
    pub fn new(mem: &'a GuestMemoryMmap, head: DescriptorChain) -> Option<Self> {
        if head.len as usize != size_of::<CtrlHdr>() {
            return None;
        }
        if !head.has_next() {
            return None;
        ...
        let mut buf = [0u8; size_of::<CtrlHdr>()];
        if mem.read_slice(&mut buf, head.addr).is_err() {
            return None;
        }
        ...
        // Could be the extra data or the ack
        let next = head.next_descriptor().unwrap();
        let data;
        let ack;
        if !next.is_write_only() {
            let mut buf = vec![0u8; next.len as _];
            if mem.read_slice(&mut buf, next.addr).is_err() {
                return None;
            }
            data = Some(buf);

            if let Some(last) = next.next_descriptor() {
                ack = last.addr;
            } else {
    ...
    pub fn set_ack(&self) {
        let buf = [VIRTIO_NET_OK];
        self.mem.write_slice(&buf, self.ack).unwrap();
    }
}
...
pub struct Net {
    ...
    /// How many pairs of queue this device manages
    pub pairs: u32,
...
impl Net {
    ...
    pub fn new(
        ...
        config_space.max_virtqueue_pairs = Self::NUM_PAIRS as u16;

        let mut queue_evts = Vec::new();
        let mut queues = Vec::new();
        let num_queues = 2 * Self::NUM_PAIRS + 1;
        for _ in 0..num_queues {
            queue_evts.push(EventFd::new(libc::EFD_NONBLOCK).map_err(NetError::EventFd)?);
            queues.push(Queue::new(NET_QUEUE_MAX_SIZE));
        }

        Ok(Net {
            id: id.clone(),
            tap_if_name: tap_if_name.to_owned(),
            pairs: Self::NUM_PAIRS as u32,
            avail_features,
            acked_features: 0u64,
            queues,
            queue_evts,
            ...
            irq_trigger: IrqTrigger::new().map_err(NetError::EventFd)?.into(),
            config_space,
            guest_mac,
    ...
    pub fn configure_mmds_network_stack(&mut self, ipv4_addr: Ipv4Addr, mmds: Arc<Mutex<Mmds>>) {
        if (self.avail_features & (1 << VIRTIO_NET_F_MQ)) != 0 {
            error!("Multi-queue conflicts with MMDS.");
            return;
        }
    ...
    pub fn process_ctrl(&mut self) -> Result<(), DeviceError> {
        let mem = self.device_state.mem().unwrap();
        let ctrl_queue = self.queues.last_mut().unwrap();

        while let Some(head) = ctrl_queue.pop_or_enable_notification() {
            let head_index = head.index;
            let ctrl_command = if let Some(cc) = CtrlCommand::new(mem, head) {
                cc
            } else {
                continue;
            };
            if ctrl_command.hdr.class == VIRTIO_NET_CTRL_MQ
                && ctrl_command.hdr.command == VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET
            {
                assert_eq!(self.pairs as u8, ctrl_command.data.as_ref().unwrap()[0]);
            }
            ctrl_command.set_ack();
            ctrl_queue
                .add_used(head_index, 0)
                .map_err(DeviceError::QueueError)?;
        }

        if ctrl_queue.prepare_kick() {
            self.irq_trigger
                .trigger_irq(IrqType::Vring)
                .map_err(|err| {
                    self.metrics.event_fails.inc();
    ...
    //    };
    //    self.tap.as_mut().unwrap().read_iovec(slice)
    //}
    ...
    /// TODO
    pub fn process_ctrl_queue_event(&mut self) {
        // Read from the control queue's eventfd to clear the notification.
        if let Err(err) = self.queue_evts.last().unwrap().read() {
            error!("Failed to read control queue eventfd: {:?}", err);
            self.metrics.event_fails.inc();
        } else {
            self.process_ctrl()
                .unwrap_or_else(|err| report_net_event_fail(&self.metrics, err));
...
pub struct Worker {
    ...
    pub epoll: Epoll,
    pub irq_trigger: Arc<IrqTrigger>,
...
impl Worker {
    pub const PROCESS_VIRTQ_RX: u64 = 0;
    pub const PROCESS_VIRTQ_TX: u64 = 1;
    pub const PROCESS_TAP_RX: u64 = 2;

    pub fn register_worker_events(&self) {
        self.epoll
            .ctl(
                ControlOperation::Add,
                self.rx_event.as_raw_fd(),
                EpollEvent::new(EventSet::IN, Self::PROCESS_VIRTQ_RX),
            )
            .unwrap();
        self.epoll
            .ctl(
                ControlOperation::Add,
                self.tx_event.as_raw_fd(),
                EpollEvent::new(EventSet::IN, Self::PROCESS_VIRTQ_TX),
            )
            .unwrap();
        self.epoll
            .ctl(
    ...
    /// Parse available RX `DescriptorChains` from the queue
    pub fn parse_rx_descriptors(&mut self) {
        while let Some(head) = self.rx_queue.pop_or_enable_notification() {
            let index = head.index;
            // SAFETY: we are only using this `DescriptorChain` here.
    ...
    pub fn process_rx_queue_event(&mut self) {
        ...
        self.resume_rx().unwrap_or_else(|err| error!("{:?}", err));
    }
    ...
    fn process_rx(&mut self) -> Result<(), DeviceError> {
        ...
        if self.rx_queue.prepare_kick() {
            self.irq_trigger
                .trigger_irq(IrqType::Vring)
                .map_err(|err| DeviceError::FailedSignalingIrq(err))?;
        }
    ...
    pub fn process_tx_queue_event(&mut self) {
        if let Err(err) = self.tx_event.read() {
            error!("Failed to get tx queue event: {:?}", err);
        }

        self.process_tx().unwrap_or_else(|err| error!("{:?}", err));
    }
    ...
    fn process_tx(&mut self) -> Result<(), DeviceError> {
        while let Some(head) = self.tx_queue.pop_or_enable_notification() {
            ...
            if unsafe {
                self.tx_buffer
                    .load_descriptor_chain(&self.mem, head)
                    .is_err()
            } {
                self.tx_queue
                    .add_used(head_index, 0)
    ...
    fn write_to_tap(&mut self) {
        let _ = self.tap.write_iovec(&self.tx_buffer);
    }
...
impl Net {
    pub fn launch_worker_threads(&mut self) {
        for i in 0..self.pairs {
            let mem = self
                .device_state
                .mem()
                .expect("Device is not activated yet.")
                .clone();
            let epoll = Epoll::new().unwrap();

            let rx_index = (i * 2) as usize;
            let tx_index = (i * 2 + 1) as usize;

            // TODO: open tap
            let id = i as usize;
            let tap = Tap::open_named(&self.tap_if_name)
                .map_err(|err| error!("{:?}", err))
                .unwrap();
            let supported_flags: u32 = Net::build_tap_offload_features(self.acked_features);
            tap.set_offload(supported_flags).unwrap();
            tap.set_vnet_hdr_size(vnet_hdr_len() as i32).unwrap();

            let rx_queue = self.queues[rx_index].clone();
            let tx_queue = self.queues[tx_index].clone();

            let rx_event = self.queue_evts.remove(0);
            let tx_event = self.queue_evts.remove(0);
            ...
            let mut worker = Worker {
                ...
                mem,
                epoll,
                irq_trigger: self.irq_trigger.clone(),

                tx_buffer: Default::default(),
                rx_buffer: RxBuffers::new().unwrap(),
            };
