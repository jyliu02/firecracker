...
impl Net {
    ...
    const PROCESS_VIRTQ_CTRL: u32 = 3;
    const PROCESS_TAP_RX: u32 = 4;
    const PROCESS_RX_RATE_LIMITER: u32 = 5;
    const PROCESS_TX_RATE_LIMITER: u32 = 6;
    ...
    fn register_runtime_events(&self, ops: &mut EventOps) {
        ...
        //}
        if let Err(err) = ops.add(Events::with_data(
            self.queue_evts.last().unwrap(),
            Self::PROCESS_VIRTQ_CTRL,
            EventSet::IN,
        )) {
            error!("Failed to register control queue event: {}", err);
        }
        //if let Err(err) = ops.add(Events::with_data(
        //    &self.tap,
        //    Self::PROCESS_TAP_RX,
    ...
    fn process(&mut self, event: Events, ops: &mut EventOps) {
        ...
        if self.is_activated() {
            match source {
                ...
                Self::PROCESS_VIRTQ_CTRL => self.process_ctrl_queue_event(),
                _ => {
